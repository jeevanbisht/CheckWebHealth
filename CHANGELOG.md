...
- Added a **staged diagnostic pipeline** (`src/core/diagnosis.mjs`) that runs **before** classification and answers, in order: (1) can the diagnostic browser be trusted? (2) is the Direct-vs-GSA comparison valid? (3) what is the root cause? It introduces a **Browser Trust Score** (0–100 → Trusted/Mostly Trusted/Questionable/Untrusted), a **Browser Environment** gate (`PASS`/`WARNING`/`FAILED`), **Path Comparison Validity** (`VALID_PATH_COMPARISON` / `TOOL_BROWSER_BLOCKED` / `INCONCLUSIVE_BOTH_PATHS_FAILED` / `MANUAL_BROWSER_REQUIRED` / `AUTH_REQUIRED` / `CLIENT_POSTURE_REQUIRED`), a primary **`AUTOMATION_OR_BROWSER_POSTURE`** diagnosis with secondary reasons, **Diagnostic Reliability** (`HIGH`/`MEDIUM`/`LOW`/`INCONCLUSIVE`), and four separated confidences (diagnosis / evidence / browser-trust / path-reliability). The catalog report now renders a per-arm **Browser Environment + Trust** panel and a per-row staged **Diagnosis** block with a recommended next action; the raw `verdict` column is kept as *evidence*.
- **`IP_REPUTATION` is now gated by the pipeline**: it is emitted only when the browser is trusted (Stage 1 `PASS`), the comparison is valid, a control path succeeds, and the evidence is a hard `403`/`451` with `_abck` passed. When **every** automated path fails (Direct + GSA) while the browser isn't trusted, the engine reports `AUTOMATION_OR_BROWSER_POSTURE` / `TOOL_BROWSER_BLOCKED` — never a network-path conclusion. The probe records a per-run browser `environment` snapshot (webdriver, engine, Edge version, client hints, cookie count) in results meta for deterministic trust scoring.
